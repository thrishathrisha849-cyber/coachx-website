 ...
 * Instructor API Requirements). The ROUTE only requires the baseline
 * `course.update` permission (so a `course_instructor` can submit
 * DRAFT→REVIEW), but transitioning specifically INTO PUBLISHED or
 * SCHEDULED additionally requires `course.publish` — checked here, body-
 * aware, since the route-level `requirePermission` middleware can't see
 * the request body's target status.
 */
export const postCourseStatus = asyncHandler(async (req: Request, res: Response) => {
  const targetStatus = req.body.status as string;
  if ((targetStatus === 'PUBLISHED' || targetStatus === 'SCHEDULED') && !roleHasPermission(req.user!.roles, 'course.publish')) {
    throw AppError.forbidden('You do not have permission to publish this course');
  }

  const course = await changeCourseStatus(req.params.id, targetStatus, req.user!.id);
  res.status(200).json(buildSuccessResponse(course));
});
