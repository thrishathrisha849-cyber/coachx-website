...
- Each `Session` row IS the live rotation state for one device/session
  — there is no separate `RefreshToken` table (see the model comment in
  `schema.prisma`).
- **Rotation**: every `/auth/refresh` call issues a brand-new refresh
  token and overwrites `refreshTokenHash` in place; the old token
  immediately stops matching any row.
- **Reuse detection**: if a presented refresh token's hash does not
  match ANY session's current hash, but the row that most recently held
  a matching hash is found in a `revoked` state, the ENTIRE
  `tokenFamily` is revoked — not just the specific token. This is the
  ...
  family is detected, forcing a fresh login.
...
| `/auth/refresh` | Rotates `refreshTokenHash` in place, same family |
